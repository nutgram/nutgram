...
it('groups middleware with nesting level 1', function ($update) {
    ...
    $bot->groupMiddleware($middleware1, function (Nutgram $bot) use (&$test) {
...
it('groups middleware with 2 nesting levels 1', function ($update) {
    ...
    $bot->groupMiddleware([$middleware1, $middleware2], function (Nutgram $bot) use (&$test) {
        $bot->onMessage(function (Nutgram $bot) use (&$test) {
            $test .= 'H2';
        })->middleware(function (Nutgram $bot, $next) use (&$test) {
            $test .= 'LM2';
...
it('groups middleware with nesting level 2', function ($update) {
    ...
    $bot->groupMiddleware($middleware1, function (Nutgram $bot) use ($middleware2, &$test) {
        $bot->groupMiddleware($middleware2, function (Nutgram $bot) use (&$test) {
            $bot->onMessage(function (Nutgram $bot) use (&$test) {
                $test .= 'H1';
            })->middleware(function (Nutgram $bot, $next) use (&$test) {
                $test .= 'LM1';
...
it('groups middleware with registerCommand', function ($update) {
    ...
    $bot->groupMiddleware($middleware1, function (Nutgram $bot) {
