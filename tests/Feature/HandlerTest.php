...
it('calls the message handler with middleware group', function ($update) {
    $bot = Nutgram::fake($update);

    $test = '';

    $reusableMiddleware = function ($bot, $next) use (&$test) {
        $test .= '[MW]';
        $next($bot);
    };
    ...
    expect($test)->toBe('[MW][MW][MW]LM1H1[MW][MW]LM2H2[MW]LM3H3');
})->with('message')->only();
