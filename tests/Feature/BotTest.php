...
it('sends file works as mocked instance', function () {
    ...
    $bot->hearText('/test')
        ...
        ->assertRaw(function (RequestData $request) {
            return $request->file('document')->getStream() instanceof StreamInterface;
        });
});
...
it('unescape unicode before sending payload', function (string $text) {
    ...
    $bot->assertRaw(function (RequestData $request) use ($text) {
        return $request->get('text') === $text;
    });
})->with([
    'Hello',
    'пример',
